...
";
  ...
  private const string NotifyFieldAttributeFullNamespace = $"{PrismAvaloniaToolkit}.{NotifyFieldAttribute}";
  private const string NotifyPropertyChangedNamespace = "System.ComponentModel.INotifyPropertyChanged";
  ...
  public void Execute(GeneratorExecutionContext context)
  {
    ...
    INamedTypeSymbol attributeSymbol = context.Compilation.GetTypeByMetadataName(NotifyFieldAttributeFullNamespace);
    INamedTypeSymbol notifySymbol = context.Compilation.GetTypeByMetadataName(NotifyPropertyChangedNamespace);
    ...
    {
      string srcClass = GenerateClass(group.Key, group.ToList(), attributeSymbol, notifySymbol, context);
  ...
  private string? GenerateClass(INamedTypeSymbol classSymbol, List<IFieldSymbol> fieldSymbols, ISymbol attr, ISymbol notify, GeneratorExecutionContext context)
  {
    // Must be top-level
    ...
    public void OnVisitSyntaxNode(GeneratorSyntaxContext context)
    {
          ...
          syntax.AttributeLists.Count > 0)
      {
        foreach (VariableDeclaratorSyntax variable in syntax.Declaration.Variables)
        {
          ...
          if (symbol.GetAttributes().Any(attr => attr.AttributeClass.ToDisplayString() == NotifyFieldAttributeFullNamespace))
          {
            Fields.Add(symbol);
          }
        }
